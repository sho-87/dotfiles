...
function telescopePickers.prettyGrepPicker(pickerAndOptions)
  ...
  options.entry_maker = function(line)
    ...
    originalEntryTable.display = function(entry)
      ...
      -- Add padding
      local desired_column_width = 30

      -- Calculate the length of the tail and coordinates combined
      local tail_coordinates_len = #tail + #coordinates

      -- Use the pad_string_align function to pad the path based on the combined length
      local padding = math.max(0, desired_column_width - tail_coordinates_len)
      local padded_path = utils.pad_string_align("", padding) .. pathToDisplay

      -- Encode text if necessary
      local text = ""
      if not options.disable_text then
        text = options.file_encoding and vim.iconv(entry.text, options.file_encoding, "utf8")
      end
      ...
      return displayer({
        { icon, iconHighlight },
        tail,
        { coordinates, "TelescopeResultsNumber" },
        { padded_path, "TelescopeResultsComment" },
        text,
      })
    end
  ...
  options.entry_maker = function(line)
    ...
    originalEntryTable.display = function(entry)
      ...
      return displayer({
        { icon, iconHighlight },
        tail,
        { utils.pad_string_align(string.format("(%d)", entry.bufnr), 10), "TelescopeResultsNumber" },
