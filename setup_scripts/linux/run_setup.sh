...
select distro in Ubuntu Kubuntu "KDE Neon" pop_os Quit;
do
  case $distro in
    Ubuntu)
      update_distro
...
git config --global user.email "simonho213@gmail.com"
...
# dotfiles
rm -f ~/.zshrc
ln -s $(builtin cd $PWD/../..; pwd)/.zshrc ~/.zshrc

rm -f ~/.config/nvim/init.vim
mkdir -p ~/.config/nvim
ln -s $(builtin cd $PWD/../..; pwd)/init.vim ~/.config/nvim/init.vim
